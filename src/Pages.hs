...
import Fonts(fntLarge, fntNormal, fntSmall, fntTiny, fntTeeny)
...
drawIcePage1 c bg = setupPage bg $ do
  drawCntText fntLarge 300 720 (characterName c) 
  drawMyText  fntSmall  32 784 (showClass c)
  drawMyText  fntSmall  32 718 (race c)
  drawRaText  fntSmall 565 784 (background c)
  drawMyText  fntSmall  32 752 (show $ alignment c)
  drawRaText  fntSmall 565 752 (playerName c)
  drawRaText  fntSmall 565 718 (show0 $ experience c)
  ...
  forM_ (zip [0..] stats) $ \(y,f) -> do
    drawCntText fntLarge 54 (252 + y * 77) (show0 $ f c)
    drawCntText fntSmall 54 (224 + y * 77) (show5 $ statBonus $ f c)
  -- Inspiration, Prof Bonus, AC, Initiative Bonus, Speed
  drawCntText fntLarge 120 650 (show5 $ inspiration c)
  drawCntText fntLarge 175 650 (show1 $ profBonus c)
  drawCntText fntLarge 250 650 (show5 $ acBase c + statBonus (dexterity c) + acBonus c)
  drawCntText fntLarge 297 650 (show5 $ statBonus (dexterity c) + initiative c + profBonus c)
  drawCntText fntLarge 348 650 (if speed c == 0 then "" else show (speed c) ++ "'")
  -- HP
  drawCntText fntLarge 240 572 (show0 $ hitPoints c)
  ...
  forM_ (zip [0..] bonuses) $ \(y, (sf,bf)) -> do
    drawCntText fntTiny 120 (521 + y * 14.2) (show5 $ saveBonus (sf c) (bf c) (profBonus c))
    drawCircle 105 (522 + y * 14.2) 3 (bf c)
  ...
  forM_ (zip [0..] skills) $ \(y,(stat, statB, skill)) -> do
    drawCntText fntTiny 120 (230 + y * 14.75) (show5 $ skillBonus (stat c) (statB c) (skill c) (profBonus c))
    drawCircle 104.8 (231.4 + y * 14.75) 3 (skill c)
  -- Hit dice
  drawCntText fntNormal 242 462 (show0 $ level c)
  drawMyText  fntNormal 262 462 (if hitDice c == 0 then "" else "d" ++ show (hitDice c))
  -- Passive perception
  drawCntText fntLarge 82 180 (if wisdom c == 0 then "" else show (10 + statBonus (wisdom c)))
  -- Money 
  let monies = [copper, silver, electrum, gold, platinum]
  mapM_ (\(x,f) -> drawCntText fntSmall (231 + x * 34) 175 (show0 $ f (moneyPouch c))) (zip [0..] monies)
  -- Proficiencies and Languges
  drawMyStrings fntTiny   35 144.8 13.6 (proficiencies c)
  -- Traits, Ideals, Bonds and Flaws
  drawMyStrings fntTiny  410 667 13.6 (personalityTraits c)
  drawMyStrings fntTiny  410 607 13.6 (ideals c)
  drawMyStrings fntTiny  410 547 13.6 (bonds c)
  drawMyStrings fntTiny  410 487 13.6 (flaws c)
  -- Features and Traits
  drawMyStrings fntTeeny 405 408 13.3 (take 29 (features c))
  -- Equipment
  drawMyStrings fntTiny 222 144.8 13.6 (equipment c)
  -- Attacks
  drawMyAttacks fntTiny 222 395 21 (attacks c)
...
drawIcePage2 c bg = setupPage bg $ do
  drawCntText fntLarge 180 760 (characterName c) 
  drawMyText  fntSmall 312 775 (show0 $ age c)
  drawMyText  fntSmall 312 741 (eyeColour c)
  drawMyText  fntSmall 406 775 (height c)
  drawMyText  fntSmall 406 741 (skinColour c)
  drawMyText  fntSmall 500 775 (weight c)
  drawMyText  fntSmall 500 741 (hairColour c)
  drawMyStrings fntTeeny  37 417 13.2 (backstory c)
  drawMyStrings fntTeeny 225 690 13.2 (take 18 (allies c))
  drawMyStrings fntTeeny 405 518 13.2 (drop 18 (allies c))
  drawMyStrings fntTeeny 225 425 13.2 (take 15 (drop 29 (features c)))
  drawMyStrings fntTeeny 405 425 13.2 (drop 44 (features c))
  drawMyStrings fntTeeny 225 183 13.2 (take 12 (treasure c))
  drawMyStrings fntTeeny 405 183 13.2 (drop 12 (treasure c))
...
drawIceSpellPage bg c = setupPage bg $ do
  drawCntText fntLarge 180 760 (spellcastingClass c) 
  drawCntText fntLarge 348 760 (spellcastingAbility c) 
  drawCntText fntLarge 435 760 (show0 $ spellSaveDC c) 
  drawCntText fntLarge 520 760 (show0 $ spellAttackBonus c) 
  drawMyStrings fntTiny   50 643 14.7 (take 8 $ spellsCantrips c)
  drawMyStrings fntTiny  140 643 14.7 (drop 8 $ spellsCantrips c)
  drawMyStrings fntTiny   50 461 14.7 (take 13 $ spellsLevel1 c)
  drawMyStrings fntTiny  140 461 14.7 (drop 13 $ spellsLevel1 c)
  ...
  drawMyStrings fntTiny  420 136 14.7 (take 7 $ spellsLevel9 c)
  drawMyStrings fntTiny  510 136 14.7 (drop 7 $ spellsLevel9 c)
  drawCntText    fntSmall  85 489    (show0 $ slotsLevel1 c)
  drawCntText    fntSmall  85 249    (show0 $ slotsLevel2 c)
  drawCntText    fntSmall 270 667    (show0 $ slotsLevel3 c)
  drawCntText    fntSmall 270 429    (show0 $ slotsLevel4 c)
  drawCntText    fntSmall 270 190    (show0 $ slotsLevel5 c)
  drawCntText    fntSmall 455 667    (show0 $ slotsLevel6 c)
  drawCntText    fntSmall 455 488    (show0 $ slotsLevel7 c)
  drawCntText    fntSmall 455 309    (show0 $ slotsLevel8 c)
  drawCntText    fntSmall 455 161    (show0 $ slotsLevel9 c)
